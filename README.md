...
The app now supports both SMTP and AWS SES for account verification and password
reset emails.
...
When deployed on AWS, the SES sender uses the default AWS credential chain, so
it can work with an EC2/ECS/Elastic Beanstalk role instead of an app password.
